...
\newpage
\section{Two-week execution sprint}

Each day is a work block, not an automatic completion date. Continue from the first failed exit check; do not bypass dependencies to protect the calendar.

\begin{tabularx}{\textwidth}{>{\bfseries}p{10mm}p{70mm}X}
\toprule
Day & Main work & Observable result \\
\midrule
1 & ConOps, scope, requirements, risks & Baselined scope, requirement IDs, risk register \\
2 & Hardware assumptions, architecture, TF/interfaces & Interface-control and mismatch list \\
3 & Environment, repository, packages, tests & Reproducible build and quality command \\
4 & Acceptance world and performance baseline & Saved, reloadable, measured scene \\
5 & ROS motion and command expiry & Correct movement and predictable stopping \\
6 & LiDAR, clock, TF, odometry & Coherent sensor data in RViz \\
7 & Odometry validation and mapping & Error results and first saved map \\
8 & Clean restart and AMCL & Localization on the saved map \\
9 & Basic Nav2 configuration & First autonomous goal \\
10 & Doorway/corridor tuning & Several repeatable routes \\
11 & C++ mission manager and tests & Automated, cancellable waypoint mission \\
12 & Fault injection, metrics, fusion decision & Predictable failures and recorded comparisons \\
13 & Frozen acceptance campaign and fixes & Traceable pass/fail results \\
14 & Regression, release, V2 gate review & Tagged V1 release or explicit remaining-work list \\
\bottomrule
\end{tabularx}

\textbf{Dependency gates}
\begin{itemize}
  \item Coherent interfaces, frames, sensors, time, and odometry before mapping.
  \item Saved-map localization before navigation tuning.
  \item Reliable single-goal navigation before mission-manager expansion.
  \item Frozen requirements and configuration before the acceptance campaign.
\end{itemize}

\textbf{If the sprint overruns:} preserve stopping behavior, fault handling, acceptance tests, evidence, and reproducibility. Defer visual polish, decorative assets, optional sensor fusion, and nonessential features.
